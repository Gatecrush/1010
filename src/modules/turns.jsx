 ...
 *
 * Now handles the case where the played card can only be used for *one* capture set.
 */
const isValidMultiCaptureSelection = (selectedItems, validCaptureOptions, playedCard, tableItems) => {
    // Basic checks
    ...
    // 1. Generate valid capture options for the table items *without* the played card
    const tableOnlyOptions = CaptureValidator.getValidCaptures(null, tableItems); // Pass null for playedCard

    // 2. Check if the selectedItems *excluding* the played card can be partitioned
    // into a combination of these table-only options
    let remainingSelectedItemIds = new Set(selectedItems.map(item => item.id));
    // Remove the played card's ID from the set, if it's present (it shouldn't be)
    // if (playedCard && playedCard.id) remainingSelectedItemIds.delete(playedCard.id);

    let currentOptions = [...tableOnlyOptions]; // Copy options
    ...
    while (progressMade && remainingSelectedItemIds.size > 0) {
        ...
        if (optionUsedIndex !== -1) {
            currentOptions.splice(optionUsedIndex, 1);
             ...
             return false; // Partitioning failed
        }
    }

    // 3. If there are any items remaining, it means the played card must be used to capture them
    // In this case, the played card must be able to capture *all* remaining items *together*
    if (remainingSelectedItemIds.size > 0) {
        const remainingItems = selectedItems.filter(item => remainingSelectedItemIds.has(item.id));
        const playedCardCapture = CaptureValidator.getValidCaptures(playedCard, tableItems);
        // Check if any of the playedCardCapture options exactly matches the remaining items
        const hasPlayedCardCapture = playedCardCapture.some(option => areItemSetsEqual(option, remainingItems));
        if (!hasPlayedCardCapture) {
            // console.log("Played card cannot capture remaining items:", remainingItems.map(i => i.id));
            return false; // Played card can't capture the remaining items
        }
    }

    // If all checks pass, the selection is valid
    return true;
};
...
export const handleCapture = (playedCard, selectedItems, currentPlayer,
    ...
    // Pass playedCard and tableItems to isValidMultiCaptureSelection
    let isSelectionValid = isValidMultiCaptureSelection(selectedItems, allValidOptions, playedCard, tableItems);
