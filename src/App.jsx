// src/App.jsx
    ...
    function App() {
      ...
      const initializeDeal = () => {
        ...
        // Check if 10 of Diamonds is on the table
        const tenOfDiamondsOnTable = table.some(card => card.suitRank === 'D10');

        if (tenOfDiamondsOnTable) {
          // Reshuffle and redeal if 10 of Diamonds is on the table
          setMessage("Reshuffling because 10 of Diamonds was dealt on the table.");
          setTimeout(() => {
            resetGame(); // Resets the game, creating a new deck and reshuffling
            initializeDeal(); // Calls initializeDeal again to start the dealing process
          }, 1500); // Delay to show the message
          return; // Important: Exit this function to prevent further execution
        }
